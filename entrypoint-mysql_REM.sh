...
# latter only show values present in conysqfig files, and not server defaults
_get_config() {
	local conf="$1"; shift
...
if [ "$1" = 'mysqld' ]; then
	...
	result=0
	output=$(mysqld --verbose --help 2> /dev/null) || result=$?
	if [ ! "$result" = "0" ]; then
...
EOF
	...
	echo "[Entrypoint] there's no file $DATADIR/mysql"
fi
